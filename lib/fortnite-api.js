...
class FortniteAPI {
     ...
     * @returns {Promise}
     */
    getWeaponsDetails(id, options = {}) {
        const lang = options.lang || this.defaultLang;
        const uri = endpoints.getWeaponsDetails(id, lang);
        return this.request(uri);
    }
    ...
    listFishes(options = {}) {
        const lang = options.lang || this.defaultLang;
        const uri = endpoints.listFishes(lang);
        return this.request(uri);
    }

    /**
     * getPlayerFishesStats()
     * Get the stats for a specific player: each fish caught is returned with the best length for that player
     *
     * @param {String} accountId
     * @returns {Promise}
     */
    getPlayerFishesStats(accountId) {
        const uri = endpoints.getPlayerFishesStats(accountId);
        return this.request(uri);
    }
}
